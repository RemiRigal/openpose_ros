...
DEFINE_double(net_resolution_dynamic,   1.,             "This flag only applies to images or custom inputs (not to video or webcam). If it is zero"
                                                        " or a negativevalue, it means that using `-1` in `net_resolution` will behave as explained in"
                                                        " its description. Otherwise, and to avoid out of memory errors, the `-1` in `net_resolution`"
                                                        " will clip to this value times the default 16/9 aspect ratio value (which is 656 width for a"
                                                        " 368 height). E.g., `net_resolution_dynamic 10 net_resolution -1x368` will clip to 6560x368"
                                                        " (10 x 656). Recommended 1 for small GPUs (to avoid out of memory errors but maximize speed)"
                                                        " and 0 for big GPUs (for maximum accuracy and speed).");
